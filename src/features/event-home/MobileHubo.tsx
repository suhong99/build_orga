...
const MobileHubo = ({ name }: { name: EventPerson }) => {
	...
	const y = useTransform(scrollYProgress, [0.4, 0.57], ['96%', '0%']);

	return (
		<Link href={`/event/${name}`} className="flex flex-col w-full ">
			<section ref={ref} className="h-[240vh] relative w-full desktop:hidden">
				<div className="sticky top-16 h-[calc(100vh-64px)] flex flex-col">
					...
					<div className="relative h-[55%] overflow-hidden pb-[7%]">
						{/* 다음녀석 */}
						...
						<motion.div style={{ y }} className={`absolute inset-0 flex flex-col items-center z-10 ${HUBO_COLOR[name].bg}`}>
							<div className="flex flex-col items-center p-5 gap-6 max-w-[400px] h-full">
								...
								<div className="flex w-full px-2.5 pt-4 justify-center h-[134px]">
									<div className="gap-3 flex text-white items-start justify-center">
								...
								<div className="flex w-full justify-between h-6 items-center mt-auto">
									<div className="flex gap-2">
										{data.dark.label.map((text) => (
											<div
												key={text}
												className={`flex items-center text-center typo-caption1 text-[#F7F7F8] bg-[#F7F7F8]/9  rounded-[6px] h-6 px-1.5`}
											>
												{text}
											</div>
										))}
